  ...
  return (
    <div style={{ minHeight: "100vh", background: C.bg, fontFamily: "'Inter', -apple-system, sans-serif", maxWidth: 480, margin: "0 auto", paddingBottom: 40 }}>
      <div style={{ background: C.white, borderBottom: `1px solid ${C.border}`, padding: "20px" }}>
        ...
        <div style={{ fontSize: 22, fontWeight: 800, color: C.navy }}>Login History</div>
        <div style={{ fontSize: 12.5, color: C.slate, marginTop: 4 }}>Most recent sign-ins to this account, newest first.</div>
      </div>
          ...
          rows.map((r, i) => (
            <div key={r.id} style={{ display: "flex", alignItems: "center", padding: "13px 0", borderBottom: `1px solid ${C.border}`, gap: 12 }}>
              <div style={{
                ...
                background: i === 0 ? C.violetLight : "#F1F5F9",
...
const CHART_COLORS = ["#7C3AED", "#F43F5E", "#10B981", "#F59E0B", "#3B82F6", "#EC4899", "#14B8A6", "#F97316", "#8B5CF6", "#0EA5E9"];
  ...
  React.useEffect(() => {
    ...
    chartRef.current = new Chart(canvasRef.current.getContext("2d"), {
      ...
      data: {
        ...
        datasets: [{ data: data.map(d => d.value), backgroundColor: data.map((_, i) => CHART_COLORS[i % CHART_COLORS.length]), borderWidth: 0 }],
      },
      options: { plugins: { legend: { display: false }, tooltip: { callbacks: { label: (ctx) => `${ctx.label}: ${formatCurrency(ctx.parsed)}` } } }, cutout: "68%" },
    });
    return () => { if (chartRef.current) chartRef.current.destroy(); };
  }, [JSON.stringify(data)]);

  return <canvas ref={canvasRef} style={{ maxHeight: 200 }}></canvas>;
}
    ...
    chartRef.current = new Chart(canvasRef.current.getContext("2d"), {
      ...
      data: {
        ...
        datasets: [{
          ...
          borderColor: C.violet, backgroundColor: "rgba(124,58,237,0.08)",
      ...
      options: {
        ...
        scales: {
          x: { grid: { display: false }, ticks: { color: C.slate, font: { size: 10 }, maxTicksLimit: 6 } },
          y: { grid: { color: C.border }, ticks: { color: C.slate, font: { size: 10 }, callback: (v) => formatCurrency(v) } },
        },
      },
    });
    return () => { if (chartRef.current) chartRef.current.destroy(); };
  ...
  if (entries.length === 0) {
    return (
      <div style={{ textAlign: "center", padding: "40px 0", color: C.slate, fontSize: 13 }}>
        Add a few transactions and your charts will show up here.
      </div>
    );
  }
  ...
  return (
    <div>
      ...
      <div style={{ background: C.white, border: `1px solid ${C.border}`, borderRadius: 14, padding: 18, marginBottom: 16 }}>
        <div style={{ fontSize: 12.5, fontWeight: 700, color: C.navy, marginBottom: 4 }}>Balance Over Time</div>
        <div style={{ fontSize: 11, color: C.slate, marginBottom: 12 }}>Running balance across every logged transaction.</div>
      ...
      <div style={{ background: C.white, border: `1px solid ${C.border}`, borderRadius: 14, padding: 18, marginBottom: 16 }}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 4 }}>
          <div style={{ fontSize: 12.5, fontWeight: 700, color: C.navy }}>By Category</div>
          <div style={{ display: "flex", gap: 6 }}>
            {["expense", "income"].map(t => (
              <button key={t} onClick={() => setBreakdownType(t)} style={{
                ...
                background: breakdownType === t ? C.navy : C.white,
                color: breakdownType === t ? "#fff" : C.slate,
                border: `1px solid ${breakdownType === t ? C.navy : C.border}`,
              }}>
                {t}
              </button>
            ))}
        ...
        {breakdown.length === 0 ? (
          <div style={{ textAlign: "center", padding: "24px 0", color: C.slate, fontSize: 12.5 }}>
            No {breakdownType} entries yet.
          </div>
        ) : (
          <div style={{ display: "flex", alignItems: "center", gap: 16, marginTop: 12 }}>
            <div style={{ flexShrink: 0, width: 130 }}>
              <DoughnutChart data={breakdown} />
            </div>
            <div style={{ flex: 1, minWidth: 0 }}>
              {breakdown.map((d, i) => (
                <div key={d.label} style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 8 }}>
                  ...
                  <div style={{ fontSize: 12, color: C.navy, flex: 1, minWidth: 0, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{d.label}</div>
                  <div style={{ fontSize: 11.5, fontWeight: 700, color: C.slate, flexShrink: 0 }}>
                    {breakdownTotal ? Math.round((d.value / breakdownTotal) * 100) : 0}%
                  </div>
                </div>
              ))}
            </div>
  ...
  React.useEffect(() => {
    ...
    chartRef.current = new Chart(canvasRef.current, {
      ...
      data: {
        ...
        datasets: [{ data: [income, expense], backgroundColor: [C.violet, C.coral], borderWidth: 0 }],
      },
      options: {
        cutout: "68%",
        responsive: true,
...
function FinanceTracker({ session, onShowHistory, onLogout }) {
  ...
  const filtered = React.useMemo(() => {
    // Already most-recent-first from the query; just apply the filter.
  ...
  const handleAdd = () => {
    ...
    setError("");
    const newRow = { user_id: userId, type: form.type, category: form.category, amount: Number(form.amount), note: form.note || null, entry_date: form.date };
    db.from("entries").insert(newRow).select().single().then(({ data, error: insertError }) => {
      if (insertError) { setError(insertError.message); return; }
      setEntries(prev => [{ id: data.id, type: data.type, category: data.category, amount: Number(data.amount), note: data.note, date: data.entry_date }, ...prev]
      ...
      <div style={{ background: C.white, borderBottom: `1px solid ${C.border}`, padding: "20px 20px 0" }}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 4 }}>
          ...
          <div style={{ display: "flex", gap: 14 }}>
            <span onClick={onShowHistory} style={{ fontSize: 11.5, fontWeight: 700, color: C.violet, cursor: "pointer" }}>History</span>
...
function App() {
  ...
  const [view, setView] = React.useState("tracker"); // 'tracker' | 'history'
  ...
  if (view === "history") return <LoginHistory userId={session.user.id} onBack={() => setView("tracker")} />;
